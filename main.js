...
function calculateDirection(cam) {
    ...
    const len = Math.sqrt(
        direct.x * direct.x + direct.y * direct.y + direct.z * direct.z,
    );
...
function moveAngledCamera() {
    let velX =
        camera.vz * Math.sin(camera.ay * RADIAN) +
        camera.vx * Math.cos(camera.ay * RADIAN);
    let velZ =
        camera.vz * Math.cos(camera.ay * RADIAN) -
        camera.vx * Math.sin(camera.ay * RADIAN);
    let velY = camera.vy;
...
function bufferProjectiles(projectileList, isComplex) {
    if (!isComplex) {
        for (let i = 0; i < projectileList.length; i++) {
            projectileList[i].bufferProjectileFaces(CUBE, false, renderList);
        }
    } else {
        for (let i = 0; i < projectileList.length; i++) {
            projectileList[i].bufferProjectileFaces(sphere, true, renderList);
        }
    }
}

// Buffers the target faces
function bufferTargets(targetList, isComplex) {
    if (!isComplex) {
        for (let i = 0; i < targetList.length; i++) {
            targetList[i].bufferTargetFaces(CUBE, false, renderList);
        }
    } else {
        for (let i = 0; i < targetList.length; i++) {
            targetList[i].bufferTargetFaces(sphere, true, renderList);
        }
    }
}
...
function updateProjectiles(projectileList) {
    ...
    for (let i = 0; i < projectileList.length; i++) {
        if (projectileList[i].y < -20) {
            continue;
        }
        newList.push(projectileList[i].updateProjectile());
    }
...
function checkProjectileCollision(projectile, targetList) {
    for (let i = 0; i < targetList.length; i++) {
        const dist = Math.sqrt(
            Math.pow(projectile.x - targetList[i].x, 2) +
                Math.pow(projectile.y - targetList[i].y, 2) +
                Math.pow(projectile.z - targetList[i].z, 2),
        );

        if (dist < projectile.size + targetList[i].size) {
            return i;
        }
    }
...
// Essentially, if a projectile hits a target, get rid of both of them
// Returns new projectile list and a list that contains all the targets that are hit
function filterProjectilesCollision(projectileList, targetList) {
    let newList = [];
    let filterList = [];

    for (let i = 0; i < projectileList.length; i++) {
        const check = checkProjectileCollision(projectileList[i], targetList);

        if (check == -1) {
            newList.push(projectileList[i]);
            continue;
        }
...
// Gets rid of the targets from the target list if it is in the filter list
function targetCollision(targetList, filterList) {
    let newList = [];
...
function rotatePoint(x, y, z) {
    ...
    return {
        x:
            Math.cos(ry) * (Math.sin(rz) * y + Math.cos(rz) * x) -
            Math.sin(ry) * z,
        y:
            Math.sin(rx) *
                (Math.cos(ry) * z +
                    Math.sin(ry) * (Math.sin(rz) * y + Math.cos(rz) * x)) +
            Math.cos(rx) * (Math.cos(rz) * y - Math.sin(rz) * x),
        z:
            Math.cos(rx) *
                (Math.cos(ry) * z +
                    Math.sin(ry) * (Math.sin(rz) * y + Math.cos(rz) * x)) -
            Math.sin(rx) * (Math.cos(rz) * y - Math.sin(rz) * x),
    };
}
...
// 1) Create two lines starting from a vertex and going to each of the two other vertices 
// 2) Cross product between them, gets line perpendicular to other two lines
// 3) Divide by the length of the normal to get a unit vector
...
function dotProductVec3(x1, y1, z1, x2, y2, z2) {
...
function checkOutsidePoints(tri, side) {
    switch (side) {
        case "zNear":
            let outPoints = 0;
            if (tri.z1 < fNear) outPoints += 1;
            if (tri.z2 < fNear) outPoints += 1;
            if (tri.z3 < fNear) outPoints += 1;
            return outPoints;
    }
}
...
function calcLighting(tri, light) {
    ...
    let similarity =
        dotProductVec3(normal.x, normal.y, normal.z, light.x, light.y, light.z) /
        1.5;
...
// Creating 10 targets, puts them to random location
for (let i = 0; i < 10; i++) {
...
// Sets the main function to run at 60 fps or once per 16.66ms
